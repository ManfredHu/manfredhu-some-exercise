...
<title>共享列表</title>
<link href="/CMS/Content/CSS/shareList.css" rel="stylesheet"
	type="text/css" />
</head>
	...
	<div id="content">
		<ul style="list-style-type: none;">
			<c:forEach var="item" items="${requestScope.shareList }">
				<li>
					...
					<p class="name">
						<span class="username">${item.user.name }</span>发布于
		...
		<div id="leftListBottom">
			...
			<c:if test="${currentPage ne 1 } ">
				<a
					href="/CMS/code/shareList.action?currentPage=${requestScope.prePage }">
					上一页 </a>
			</c:if>
			...
			<c:if test="${currentPage ne pageNum } ">
				<a
					href="/CMS/code/shareList.action?currentPage=${requestScope.nextPage }">
					下一页 </a>
			</c:if>
		</div>
	</div>

...
let client = (function () {
  ...
  if (/AppleWebKit\/(\S+)/.test(ua)) {
    ...
    if (/Chrome\/(\S+)/.test(ua)) {
      // 确定是不是Chrome,第一个是Chrome考虑到现在市场Chrome份额比Opera多，能减少代码执行时间
      browser.ver = RegExp['$1'];
      browser.chrome = parseFloat(browser.ver);
    } else if (/Version\/(\S+)/.test(ua)) {
      // 确定是不是高版本（3+）的Safari,同样Safari比Opera份额多
      browser.ver = RegExp['$1'];
  ...
  let p = navigator.platform; // 判断操作系统
  ...
  if (system.win) {
    if (/Win(?:dows )?([^do]{2})\s?(\d+\.\d+)?/.test(ua)) {
      // 从XP系统开始统一Windows NT 5.1(Opera<7除外)
      if (RegExp['$1'] === 'NT') {
        system.win =
          {
            '5.0': '2000',
...
let browserName = ''; // 保存当前使用的浏览器品牌信息
let browserVer = 0; // 保存当前使用的浏览器版本信息
...
let useEngine = ''; // 保存当前浏览器引擎（内核）名称
let engineVer = 0; // 保存当前使用的浏览器引擎版本
